...
impl SwiftLinker {
    ...
    pub fn link(self) {
        ...
        for package in self.packages {
            ...
            let platform_path_output = Command::new("xcrun")
                .args(["--sdk", &rust_target.sdk.to_string(), "--show-sdk-platform-path"])
                .output()
                .unwrap();
            let platform_path = if platform_path_output.status.success() {
                Some(String::from_utf8_lossy(&platform_path_output.stdout).trim().to_string())
            } else {
                None
            };
            ...
            // For iOS, pass an explicit destination triple so SwiftPM resolves xcframework
            // slices for iOS instead of host macOS.
            ...
            let search_path = find_library_search_path(&out_path, &package.name, configuration, arch)
                .unwrap_or_else(|| {
                    // Keep backward-compatible fallback for older SwiftPM layouts.
                    out_path
            ...
            // Swift object files may request auto-linked Apple frameworks.
            // Expose the same directory as a framework search path so ld can resolve them.
            println!("cargo:rustc-link-search=framework={}", search_path.display());
            if let Some(platform_path) = &platform_path {
                let developer_frameworks = Path::new(platform_path)
                    .join("Developer")
                    .join("Library")
                    .join("Frameworks");
                let developer_private_frameworks = Path::new(platform_path)
                    .join("Developer")
                    .join("Library")
                    .join("PrivateFrameworks");
                if developer_frameworks.exists() {
                    println!(
                        "cargo:rustc-link-search=framework={}",
                        developer_frameworks.display()
                    );
                }
                if developer_private_frameworks.exists() {
                    println!(
                        "cargo:rustc-link-search=framework={}",
                        developer_private_frameworks.display()
                    );
                }
            }
            for framework in list_framework_names(&search_path) {
...
fn find_library_search_path(
    out_path: &Path,
    package_name: &str,
    configuration: &str,
    arch: &str,
) -> Option<PathBuf> {
    let lib_name = format!("lib{package_name}.a");
    let legacy = out_path
        .join(format!("{arch}-apple-macosx"))
        .join(configuration);

    if legacy.join(&lib_name).exists() {
        return Some(legacy);
    }

    find_library_parent_dir(out_path, configuration, &lib_name)
}

fn find_library_parent_dir(root: &Path, configuration: &str, lib_name: &str) -> Option<PathBuf> {
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_dir() {
                if path.file_name().and_then(|name| name.to_str()) == Some(configuration) {
...
fn list_framework_names(dir: &Path) -> Vec<String> {
    ...
    for entry in entries.flatten() {
        ...
        if let Some(framework) = name.strip_suffix(".framework") {
